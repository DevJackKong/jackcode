...
import { runAllE2EScenarios, runErrorRecoveryScenario, runRefactoringScenario, runSimpleModificationScenario, runTddScenario, cleanupScenarioProject, } from './runner.ts';
...
test('Scenario 4: error recovery detects a failure and repairs it automatically', async () => {
    ...
    try {
        ...
        assert.ok(result.observations.attempts >= 2);
    }
    finally {
        cleanupScenarioProject(result.projectDir);
    }
