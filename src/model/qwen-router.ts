...
export type QwenErrorType = 'rate_limit' | 'timeout' | 'context_overflow' | 'tool_error' | 'transient' | 'fatal';
...
export interface QwenProvider {
  ...
  stream?(request: QwenPreparedRequest, onChunk: (chunk: string) => void): Promise<QwenProviderResponse>;
}
...
export class QwenExecutorRouter {
  ...
  private readonly policyDecisionCache = new Map<string, RoutingDecision>();
  private cacheHits = 0;
  private cacheMisses = 0;
  private slotSequence = 0;
  private metrics: RouterMetrics;

  constructor(config: Partial<QwenRouterConfig> = {}, dependencies: QwenRouterDependencies = {}) {
    ...
    for (const model of Object.keys(MODEL_PROFILES) as QwenModelId[]) this.poolLoad.set(model, 0);
  }
  ...
  async batchRoute(requests: QwenRouteRequest[]): Promise<QwenRouteResult[]> {
    if (requests.length > this.config.maxBatchSize) throw new Error(`Batch size ${requests.length} exceeds maximum ${this.config.maxBatchSize}`);
    ...
    for (const [index, request] of requests.entries()) {
      ...
      if (inflight.size >= this.config.maxConcurrency) await Promise.race(inflight);
    }
  ...
  classifyError(error: unknown): QwenErrorType {
    ...
    if (normalized.includes('context') || normalized.includes('token') || normalized.includes('maximum length')) return 'context_overflow';
    if (normalized.includes('tool')) return 'tool_error';
    if (normalized.includes('temporary') || normalized.includes('transient') || normalized.includes('unavailable')) return 'transient';
    return 'fatal';
  }
  ...
  private async acquireSlot(priority: RoutePriority, requestTimeoutMs?: number): Promise<ExecutionSlot> {
    ...
    while (this.activeSlots.size >= this.config.maxConcurrency) {
      if (Date.now() >= deadline) throw new Error('timeout acquiring execution slot');
      await this.sleep(25);
    }
    const acquiredAt = Date.now();
    const slot: ExecutionSlot = { id: `slot_${acquiredAt}_${this.slotSequence++}`, acquiredAt, expiresAt: acquiredAt + effectiveTimeoutMs };
  ...
  private getPolicyDecision(request: ExtendedQwenRouteRequest, contextTokens: number): RoutingDecision | undefined {
    if (!this.policy?.selectModel) return undefined;
  ...
  private pickByCapability(contextTokens: number, request: ExtendedQwenRouteRequest): QwenModelId {
    if (request.tools?.length) return contextTokens > MODEL_PROFILES['qwen-coder'].contextWindow ? 'qwen-3.6' : 'qwen-coder';
  ...
  private inferTaskType(request: ExtendedQwenRouteRequest): string {
    ...
    if (!operation) return 'simple_edit';
    if (operation.type === 'refactor' || /refactor/i.test(operation.description)) return 'refactor';
    if (/fix|bug|repair|regression/i.test(operation.description)) return 'build_fix';
    if (/implement|add|create|feature/i.test(operation.description)) return 'multi_file_change';
    return 'simple_edit';
  }
  ...
  private optimizeContext(request: ExtendedQwenRouteRequest, window: number, requestedOutputTokens?: number): string {
    ...
    if (estimateTokens(content) <= promptBudget) return content;
    ...
    for (const item of scored) {
      ...
      if (used + blockTokens > promptBudget && kept.length > 0) continue;
  ...
  private scoreContextBlock(block: string, index: number, ops: string): number {
    ...
    let score = 0;
    if (/^import\s|^export\s/m.test(block)) score += 9;
    if (/interface\s+|type\s+|class\s+|function\s+|const\s+\w+\s*=\s*\(/m.test(block)) score += 8;
    if (/describe\(|it\(|test\(/m.test(block)) score += 7;
    if (/\.test\.|\.spec\./.test(text)) score += 6;
    if (/recent|modified|changed/.test(text)) score += 4;
    if (ops.split(/\s+/).some((token) => token.length > 3 && text.includes(token))) score += 5;
    if (/console\.log|debugger|temporary|todo/i.test(block)) score -= 3;
    score += Math.max(0, 3 - index * 0.1);
    return score;
  }

  private buildSystemPrompt(request: ExtendedQwenRouteRequest, policyDecision?: RoutingDecision): string {
  ...
  private buildUserPrompt(request: ExtendedQwenRouteRequest, optimizedContext: string, policyDecision?: RoutingDecision): string {
    return [
  ...
  private async executeSingle(request: ExtendedQwenRouteRequest, prepared: QwenPreparedRequest, _slot: ExecutionSlot, span: SpanLike): Promise<QwenRouteResult> {
  ...
  private async executeBatch(request: ExtendedQwenRouteRequest, prepared: QwenPreparedRequest, _slot: ExecutionSlot, span: SpanLike): Promise<QwenRouteResult> {
    ...
    for (let index = 0; index < operations.length; index += batchSize) {
      ...
      const chunkResults = await Promise.all(chunk.map(async (operation) => {
        const opRequest: ExtendedQwenRouteRequest = { ...request, operations: [operation], stream: false };
  ...
  private async executeWithRetry(prepared: QwenPreparedRequest, request: ExtendedQwenRouteRequest, span: SpanLike): Promise<QwenProviderResponse> {
    ...
    while (retryCount <= this.config.retryLimit) {
      ...
      try {
        ...
        if (!this.shouldRetry(classification, retryCount)) break;
        retryCount += 1;
        const failedModel = activeModel;
        if (classification === 'rate_limit' || classification === 'timeout') activeModel = this.selectFallbackModel(activeModel, prepared, request);
  ...
  private async invokeProvider(prepared: QwenPreparedRequest, request: ExtendedQwenRouteRequest): Promise<QwenProviderResponse> {
  ...
  private shouldRetry(type: QwenErrorType, retryCount: number): boolean {
    if (retryCount >= this.config.retryLimit) return false;
    return type === 'rate_limit' || type === 'timeout' || type === 'transient';
  }

  private selectFallbackModel(activeModel: QwenModelId, prepared: QwenPreparedRequest, request: ExtendedQwenRouteRequest): QwenModelId {
    if (this.canHandleWithModel(this.config.fallbackModel, prepared.inputTokens, request.maxOutputTokens)) return this.config.fallbackModel;
    if (activeModel !== 'qwen-3.6' && this.canHandleWithModel('qwen-3.6', prepared.inputTokens, request.maxOutputTokens)) return 'qwen-3.6';
    return activeModel;
  }
...
export function createQwenRouter(config?: Partial<QwenRouterConfig>, dependencies?: QwenRouterDependencies): QwenExecutorRouter {
