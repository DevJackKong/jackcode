...
export class ModelPolicyEngine {
  ...
  private performance: Map<string, ModelPerformanceStats> = new Map();
  ...
  checkBudget(cost: number, task?: TaskContext): BudgetStatus {
    ...
    if (blocked) {
      return { allowed: false, reason: blocked.reason, violatedWindow: blocked.window, projectedSpend: blocked.projectedSpend };
    }
  ...
  recordOutcome(taskType: string, model: ModelTier, success: boolean, latencyMs: number): void {
    ...
    current.averageLatencyMs = current.attempts === 0
      ? latencyMs
      : ((current.averageLatencyMs * current.attempts) + latencyMs) / (current.attempts + 1);
    current.attempts += 1;
  ...
  enforceLimits(decision: RoutingDecision, task?: TaskContext, complexity?: ComplexityScore, ruleResults: RuleResult[] = []): RoutingDecision {
    ...
    while (candidate !== decision.selectedModel) {
      const adjustedTokens = task ? this.estimateOptimizedTokenUsage(task, candidate, ruleResults) : decision.estimatedTokens;
      ...
      if (candidateBudget.allowed) {
        ...
        return {
          ...
          alerts: this.buildAlertsForTask(task),
          reasoning: `${this.buildReasoning(complexity ?? 'medium', ruleResults, candidate, task)} | Downgraded due to ${budget.reason}`,
        };
      }
    ...
    return { ...decision, fallbackOnFailure: false, mode: 'blocked', budgetStatus: budget, alerts: this.buildAlertsForTask(task), reasoning: `${decision.reasoning} | WARNING: ${budget.reason}` };
  }
  ...
  validatePolicyConfig(config: PolicyConfig): void {
    ...
    if (new Set(escalationChain).size !== escalationChain.length || escalationChain.length === 0) throw new Error('Escalation chain must contain unique models');
    for (const model of escalationChain) {
      if (!(model in MODEL_CAPABILITIES)) throw new Error(`Unknown model in escalation chain: ${model}`);
    }
  ...
  private mergeConfig(config: Partial<PolicyConfig>): PolicyConfig {
    return {
      ...
      policy: {
        ...
        escalationChain: config.policy?.escalationChain ?? DEFAULT_POLICY_CONFIG.policy.escalationChain,
      },
      warningThresholds: {
  ...
  private generateTaskSignature(task: TaskContext): string {
    ...
    return [task.taskType, files, intent, complexity, String(tokenEstimate), reasoning, task.batchable ? 'batchable' : 'single', String(task.batchSize ?? 1), task.overrideModel ?? 'no_override'].join(':');
  }
  ...
  private evaluateRules(task: TaskContext): RuleResult[] {
    return this.rules.filter((rule) => rule.condition(task)).map((rule) => ({ ruleName: rule.name, priority: rule.priority, action: rule.action(task) }));
  }
  ...
  private pickModelTier(task: TaskContext, complexity: ComplexityScore, rules: RuleResult[]): ModelTier {
    ...
    if (forcedRule) {
      const forcedModel = forcedRule.action.modelPreference.find((model) => this.canModelHandleTask(model, task));
      if (forcedModel) return this.applyPerformanceBias(task, forcedModel, forcedRule.action.modelPreference);
    }

    const preferredModels = rules.flatMap((result) => result.action.modelPreference);
    for (const model of preferredModels) {
      if (this.canModelHandleTask(model, task)) return this.applyPerformanceBias(task, model, preferredModels);
    }

    const pressure = this.getBudgetPressure();
    const underBudgetPressure = pressure.session >= this.config.warningThresholds.session || pressure.daily >= this.config.warningThresholds.daily || pressure.weekly >= this.config.warningThresholds.weekly || pressure.monthly >= this.config.warningThresholds.monthly;
  ...
  private applyPerformanceBias(task: TaskContext, candidate: ModelTier, fallbacks: ModelTier[]): ModelTier {
    const currentSuccess = this.getModelSuccessRate(task.taskType, candidate);
    if (currentSuccess === null || currentSuccess >= 0.6) return candidate;
    for (const model of fallbacks) {
      if (model === candidate || !this.canModelHandleTask(model, task)) continue;
      const success = this.getModelSuccessRate(task.taskType, model);
      if (success !== null && success > currentSuccess + 0.15) return model;
    }
    return candidate;
  }
  ...
  private buildTrends(): CostReport['trends'] {
    return { daily: this.aggregateTrend('day'), weekly: this.aggregateTrend('week'), monthly: this.aggregateTrend('month') };
  }
  ...
  private forecastUsage(): CostReport['forecast'] {
    return { day: this.computeBurnRate('day'), week: this.computeBurnRate('week') * 7, month: this.computeBurnRate('month') * 30 };
  }

  private computeBurnRate(window: 'day' | 'week' | 'month'): number {
    if (this.costHistory.length === 0) return 0;
    const now = Date.now();
    const msWindow = window === 'day' ? 24 * 60 * 60 * 1000 : window === 'week' ? 7 * 24 * 60 * 60 * 1000 : 30 * 24 * 60 * 60 * 1000;
  ...
  private toCsv(): string {
    const rows = [
      ...
      ...this.costHistory.map((entry) => [entry.taskId, entry.sessionId, entry.taskType, entry.model, String(entry.inputTokens), String(entry.outputTokens), entry.costUsd.toFixed(6), String(entry.latencyMs), String(entry.timestamp)].join(',')),
    ];
    return rows.join('\n');
  }
  ...
  private buildAlertsForTask(task?: TaskContext): PolicyAlert[] {
    ...
    if (task?.overrideModel) {
      alerts.push({ severity: 'info', code: 'policy.override', message: `Manual override requested: ${task.overrideModel}`, createdAt: Date.now() });
    }
    return alerts;
  }
  ...
  private pushAlert(severity: PolicyAlert['severity'], code: string, message: string, window?: BudgetWindow): void {
    this.alerts = this.mergeAlertLists(this.alerts, [{ severity, code, message, window, createdAt: Date.now() }]);
  }

  private mergeAlertLists(existing: PolicyAlert[], incoming: PolicyAlert[]): PolicyAlert[] {
    const merged = new Map<string, PolicyAlert>();
    for (const alert of [...existing, ...incoming]) merged.set(`${alert.code}:${alert.window ?? 'none'}`, alert);
  ...
  private collectOptimizations(task: TaskContext, model: ModelTier, ruleResults: RuleResult[]): OptimizationAction[] {
    ...
    for (const rule of ruleResults) {
      for (const optimization of rule.action.optimizations ?? []) optimizations.add(optimization);
    }
  ...
  private shouldBatch(task: TaskContext, model: ModelTier, ruleResults: RuleResult[]): boolean {
    if (!MODEL_CAPABILITIES[model].supportsBatching) return false;
    if (task.batchable && (task.batchSize ?? 0) >= this.config.optimization.batchMinItems) return true;
  ...
  private addOptimization(optimizations: OptimizationAction[] | undefined, optimization: OptimizationAction): OptimizationAction[] {
  ...
  private getEffectiveLimits(task?: TaskContext): PolicyConfig['policy']['costLimits'] {
    const perTask = Math.min(this.config.policy.costLimits.perTask, task?.maxCostUsd ?? this.config.policy.costLimits.perTask, task && this.taskBudgets.has(task.taskId) ? this.taskBudgets.get(task.taskId) ?? this.config.policy.costLimits.perTask : this.config.policy.costLimits.perTask);
    return { ...this.config.policy.costLimits, perTask };
  }
