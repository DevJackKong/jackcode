...
export class DefaultStreamHandler {
    chunks = [];
    thinking = false;
    onChunk(chunk) {
...
export function createChatSession(config, options = {}) {
    return {
...
export function addMessage(session, role, content, metadata) {
    const message = {
        role,
        content,
        timestamp: Date.now(),
...
export function colorize(text, color, theme) {
...
export function formatCodeBlocks(content, theme, compact = false) {
...
export function highlightInput(inputText, theme) {
...
export function renderProgress(label, current = 0, total = 0, theme = 'auto') {
...
export function renderError(error, theme = 'auto') {
    const message = error instanceof Error ? error.message : error;
    return colorize(`✖ ${message}`, 'red', theme);
}
export function renderStatusLine(session, state, theme = 'auto') {
    const segments = [
        `Session ${session.id}`,
        `Mode ${session.mode}`,
...
export function getSessionDirectory(cwd = process.cwd()) {
    return resolve(cwd, SESSION_DIR_NAME);
}
export function getSessionFilePath(sessionId, cwd = process.cwd()) {
...
export function exportSession(session, filePath) {
...
export function resumeLatestSession(cwd = process.cwd()) {
...
export function createSlashCommands(customCommands = []) {
...
export function buildHelpText(commands) {
    return [...commands.values()]
        .sort((a, b) => a.name.localeCompare(b.name))
        .map((command) => `/${command.name.padEnd(10)} ${command.description}`)
        .join('\n');
}
export async function executeSlashCommand(commandName, args, context) {
...
export function createCompleter(commands) {
...
export async function startRepl(session, options = {}) {
...
export function resolveInputFilePath(inputPath, cwd = process.cwd()) {
