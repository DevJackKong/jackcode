...
export { RetryManager, CircuitBreaker, CircuitBreakerOpenError, SafetyGuardian, RecoveryEngine, RecoveryMonitor, RecoveryError, TransientRecoveryError, PermanentRecoveryError, SafetyRecoveryError, TimeoutRecoveryError, RollbackManager, recoveryEngine, createRecoveryEngine, } from './recovery.js';
import { DEFAULT_VERIFIER_CONFIG } from '../types/reviewer.js';
export class GPT54VerifierRepairer {
    config;
    hooks = [];
    verificationHistory = new Map();
    verificationBriefs = new Map();
    modelClient;
    ...
    async verify(context) {
        ...
        if (modelAssessment?.intentFulfilled === false)
            dimensionResults.intent_match = false;
        if (typeof modelAssessment?.qualityScore === 'number' && modelAssessment.qualityScore < 0.8)
            dimensionResults.code_quality = false;
    ...
    getVerificationBrief(taskId) {
        return this.verificationBriefs.get(taskId);
    }
    async assessQuality(changes) {
    ...
    async applyPolishFixes(minorIssues) {
    ...
    async repair(context, maxAttempts = 3) {
        const attempts = [];
    ...
    async validateIntent(context) {
        ...
        const fulfilled = context.changes.length > 0 && implementationSignal && (semanticSignals >= 1 ||
            tokens.length === 0 ||
    ...
    createQualityIssues(context, report) {
        const issues = [];
        const filePath = context.changes[0]?.path || 'unknown';
        if (!report.styleCompliant) {
            issues.push({ dimension: 'code_quality', severity: 'low', description: 'Code style drifts from project conventions.', location: { filePath }, suggestion: 'Normalize formatting, line length, and debug leftovers.', autoFixPatch: this.buildStylePatch(context.changes[0]) });
        }
    ...
    createSafetyIssues(context, report) {
        const issues = [];
        const filePath = context.changes[0]?.path || 'unknown';
    ...
    makeDecision(dimensionResults, brief) {
        ...
        if (hasBlockingSeverity)
            return 'repair';
        if (brief.approvedWithSuggestions)
            return 'approve';
    ...
    calculateConfidence(dimensionResults, issues, modelConfidence) {
    ...
    generateSummary(context, decision, issues, quality, brief) {
    ...
    buildVerificationBrief(context, issues, dimensionResults) {
        const criteria = [
    ...
    async executeHooks(_context, _result) {
    ...
    semanticIntentSignals(context) {
        let signals = 0;
        if (context.changes.length > 0)
            signals += 1;
        if (context.testResults.length > 0)
            signals += 1;
        if (/fix|bug|repair/i.test(context.intent) && context.attemptHistory.length >= 0)
            signals += 1;
        return signals;
    }
    buildRepairPatch(context, issue) {
    ...
    buildStylePatch(change) {
        if (!change?.newContent)
            return undefined;
        return this.createSyntheticPatch(change.path, this.normalizeLines(change.newContent));
    }
    createSyntheticPatch(targetPath, lines) {
    ...
    async runModelVerification(context) {
        if (!this.modelClient)
            return null;
    ...
    buildVerificationPrompt(context) {
        const payload = {
            taskId: context.taskId,
    ...
    countChar(value, char) {
        return (value.match(new RegExp(`\\${char}`, 'g')) ?? []).length;
    }
    hasOddUnescapedQuotes(value, quote) {
        let count = 0;
...
export function createGPT54Verifier(config, modelClient) {
