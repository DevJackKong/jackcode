...
export class JackClawCollaborationAdapter {
    ...
    consensusByTaskId = new Map();
    handoffsByTaskId = new Map();
    assignedTasksByNode = new Map();
    runningCount = 0;
    sequenceNumber = 0;
    messageSequence = 0;
    ...
    async waitFor(handle) {
        ...
        if (TERMINAL_STATUSES.has(current.status)) {
            return this.completedResults.get(handle.id)
                ?? this.buildResult(current, current.taskId, current.status);
        }
    ...
    heartbeatNode(nodeId, loadScore) {
    ...
    async sendMessage(fromNodeId, toNodeId, payload, options = {}) {
    ...
    async broadcast(fromNodeId, payload, channel = 'broadcast') {
        this.ensureNode(fromNodeId);
    ...
    async acquireLock(resourceId, nodeId, ttlMs = this.config.deadlockTimeoutMs ?? 5_000) {
    ...
    async buildConsensus(taskId, nodeIds, votes) {
    ...
    resolveConflict(taskId, contenders) {
    ...
    async handoffTask(taskId, toNodeId, reason = 'rebalancing', stateSnapshot) {
        const handleId = this.taskIdToHandleId.get(taskId);
        if (!handleId) {
    ...
    async executeSubagent(handleId) {
        const state = this.taskStates.get(handleId);
        if (!state) {
            return;
        }
    ...
    resolveWaiters(handleId, result) {
    ...
    selectNode(task, preferredNodeId) {
    ...
    nodePressure(node, taskPriority) {
    ...
    assignTaskToNode(nodeId, taskId) {
        const assignments = this.assignedTasksByNode.get(nodeId) ?? new Set();
        assignments.add(taskId);
        this.assignedTasksByNode.set(nodeId, assignments);
    }
    unassignTaskFromNode(nodeId, taskId) {
    ...
    findWorkStealingDonor(requestingNodeId) {
    ...
    getTaskStateByTaskId(taskId) {
        const handleId = this.taskIdToHandleId.get(taskId);
        return handleId ? this.taskStates.get(handleId) : undefined;
    }
    preventDeadlock(nodeId, resourceId) {
        const awaiting = this.dependencyGraph.get(nodeId) ?? new Set();
    ...
    getTaskTimeout(handleId) {
        const state = this.taskStates.get(handleId);
        return state?.task.timeout ?? this.config.defaultTimeout;
    }
}
export function createCollaborationAdapter(config, dependencies) {
